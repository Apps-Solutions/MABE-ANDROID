...
public class QualityProblemType extends Table {

    public static final String TAG = "quality_problem_type_table";

    //Table Name
    public static final String TABLE = "quality_problem_type";
    //***Datos de Persona
    ...
    public static void removeAll(Context context){
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                QualityProblemType.delete(context,id);
            }
        }
        cursor.close();
    }
    ...
    public static void insertQualityProblemsFromJSONArray(Context context, JSONArray array){
        if (QualityProblemType.getAll(context) != null )    // Products stored in database
            QualityProblemType.removeAll(context);
        for (int i = 0; i < array.length(); i++) {
            try {
                JSONObject obj = array.getJSONObject(i);
                //Context context, String id,String description,String timestamp,String status
                QualityProblemType.insert(context,obj.getString("id_quality_problem"),obj.getString("qp_quality_problem"),
                                    obj.getString("qp_timestamp"),obj.getString("qp_status"));
                //Log.d(TAG, "inserting");
