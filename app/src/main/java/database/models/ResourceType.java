...
public class ResourceType extends Table {

    public static final String TAG = "resource_type_table";

    //Table Name
    public static final String TABLE = "resource_type";
    //***Datos de Persona
    ...
    public static void removeAll(Context context){
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                ResourceType.delete(context,id);
            }
        }
        cursor.close();
    }
    ...
    public static void insertResourcesFromJSONArray(Context context, JSONArray array){
        if (ResourceType.getAll(context) != null )    // Products stored in database
            ResourceType.removeAll(context);
        for (int i = 0; i < array.length(); i++) {
            try {
                JSONObject obj = array.getJSONObject(i);
                //Context context, String id,String description,String timestamp,String status
                ResourceType.insert(context,obj.getString("id_resource_type"),obj.getString("rt_resource_type"),
                                    obj.getString("rt_timestamp"),obj.getString("rt_status"));
                //Log.d(TAG, "inserting");
