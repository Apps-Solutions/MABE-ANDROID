...
public class Promotion extends Table {

    public static final String TAG = "promotion_table";
    ...
    public static void insertPromotionsFromJSONArray(Context context, JSONArray array){
        if (Promotion.getAll(context) != null )    // Products stored in database
            Promotion.removeAll(context);
        for (int i = 0; i < array.length(); i++) {
            try {
                JSONObject obj = array.getJSONObject(i);
                //Context context, String id,String description,String timestamp,String status
                Promotion.insert(context,obj.getString("id_promotion_type"),obj.getString("pt_promotion_type"),
                        obj.getString("pt_timestamp"),obj.getString("pt_status"));
                //Log.d(TAG, "inserting");
